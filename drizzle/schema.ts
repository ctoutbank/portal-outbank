import { pgTable, unique, serial, varchar, boolean, timestamp, char, integer, foreignKey, bigint, numeric, date, text, uuid, time, jsonb, index, pgEnum } from "drizzle-orm/pg-core"
...
// =====================================================
// Tabela MCC - Catálogo de Merchant Category Codes
// Padrão ISO 18245 para classificação de estabelecimentos
// =====================================================

export const nivelRiscoEnum = pgEnum("nivel_risco", ["baixo", "medio", "alto"]);
...
export const permissionCategories = pgTable("permission_categories", {
        ...
        foreignKey({
                columns: [table.idCustomer],
                foreignColumns: [customers.id],
                name: "permission_categories_id_customer_fkey"
        }).onDelete("cascade"),
        index("permission_categories_id_customer_idx").on(table.idCustomer),
]);
...
export const categoryFunctions = pgTable("category_functions", {
        ...
        foreignKey({
                ...
                name: "category_functions_id_category_fkey"
        }).onDelete("cascade"),
        unique("category_functions_category_function_unique").on(table.idCategory, table.functionCode),
        index("category_functions_id_category_idx").on(table.idCategory),
]);
...
export const userCategories = pgTable("user_categories", {
        ...
        foreignKey({
                columns: [table.idUser],
                foreignColumns: [users.id],
                name: "user_categories_id_user_fkey"
        }).onDelete("cascade"),
        foreignKey({
                columns: [table.idCategory],
                foreignColumns: [permissionCategories.id],
                name: "user_categories_id_category_fkey"
        }).onDelete("cascade"),
        unique("user_categories_user_category_unique").on(table.idUser, table.idCategory),
        index("user_categories_id_user_idx").on(table.idUser),
        index("user_categories_id_category_idx").on(table.idCategory),
]);

// Settlement status enum for repasse
...
export const userMonthlySettlements = pgTable("user_monthly_settlements", {
        ...
        commissionValue: numeric("commission_value", { precision: 18, scale: 2 }).default("0"),
        status: varchar({ length: 30 }).default("pending_consolidation"),
        invoiceDeadline: date("invoice_deadline"),
        paymentDeadline: date("payment_deadline"),
        paidAt: timestamp("paid_at", { mode: 'string' }),
        paidByUserId: bigint("paid_by_user_id", { mode: "number" }),
        accumulatedFromId: bigint("accumulated_from_id", { mode: "number" }),
        active: boolean().default(true),
        dtinsert: timestamp({ mode: 'string' }).default(sql`CURRENT_TIMESTAMP`),
        dtupdate: timestamp({ mode: 'string' }).default(sql`CURRENT_TIMESTAMP`),
}, (table) => [
        foreignKey({
                columns: [table.idUser],
                foreignColumns: [users.id],
                name: "user_monthly_settlements_id_user_fkey"
        }).onDelete("cascade"),
        foreignKey({
                columns: [table.idCustomer],
                foreignColumns: [customers.id],
                name: "user_monthly_settlements_id_customer_fkey"
        }).onDelete("cascade"),
        foreignKey({
                columns: [table.paidByUserId],
                foreignColumns: [users.id],
                name: "user_monthly_settlements_paid_by_user_id_fkey"
        }),
        foreignKey({
                columns: [table.accumulatedFromId],
                foreignColumns: [table.id],
                name: "user_monthly_settlements_accumulated_from_id_fkey"
        }),
        unique("user_monthly_settlements_user_customer_month_year_unique").on(table.idUser, table.idCustomer, table.month, table.year),
        index("user_monthly_settlements_id_user_idx").on(table.idUser),
        index("user_monthly_settlements_id_customer_idx").on(table.idCustomer),
        index("user_monthly_settlements_month_year_idx").on(table.month, table.year),
        index("user_monthly_settlements_status_idx").on(table.status),
]);
...
export const settlementInvoices = pgTable("settlement_invoices", {
        ...
        invoiceValue: numeric("invoice_value", { precision: 18, scale: 2 }),
        issuerCnpj: varchar("issuer_cnpj", { length: 20 }),
        ...
        sefazResponse: jsonb("sefaz_response"),
        ...
        foreignKey({
                columns: [table.idSettlement],
                foreignColumns: [userMonthlySettlements.id],
                name: "settlement_invoices_id_settlement_fkey"
        }).onDelete("cascade"),
        index("settlement_invoices_id_settlement_idx").on(table.idSettlement),
        index("settlement_invoices_access_key_idx").on(table.accessKey),
        index("settlement_invoices_validation_status_idx").on(table.validationStatus),
]);

// Tipos para Multi-ISO
...
export type InsertPermissionCategory = typeof permissionCategories.$inferInsert;
export type SelectPermissionCategory = typeof permissionCategories.$inferSelect;
export type InsertCategoryFunction = typeof categoryFunctions.$inferInsert;
export type SelectCategoryFunction = typeof categoryFunctions.$inferSelect;
export type InsertUserCategory = typeof userCategories.$inferInsert;
export type SelectUserCategory = typeof userCategories.$inferSelect;

// Tipos para Repasse
export type InsertUserMonthlySettlement = typeof userMonthlySettlements.$inferInsert;
export type SelectUserMonthlySettlement = typeof userMonthlySettlements.$inferSelect;
export type InsertSettlementInvoice = typeof settlementInvoices.$inferInsert;
export type SelectSettlementInvoice = typeof settlementInvoices.$inferSelect;
