...
interface MerchantsSearchInputProps {
  suggestions: MerchantSuggestion[];
}

export function MerchantsSearchInput({ suggestions }: MerchantsSearchInputProps) {
  ...
  const filteredSuggestions = useMemo(() => {
    ...
    return suggestions
      .filter((suggestion) => {
        ...
        const idDocument = suggestion.idDocument?.toLowerCase() || "";
        return (
          name.includes(searchTerm) ||
          corporateName.includes(searchTerm) ||
          idDocument.includes(searchTerm)
        );
      })
      .slice(0, 10);
  }, [value, suggestions]);
  ...
  // Atualizar valor quando searchParams mudar externamente
  useEffect(() => {
    const establishmentParam = searchParams?.get("establishment") || "";
    if (establishmentParam !== value) {
      setValue(establishmentParam);
    }
  }, [searchParams]);
              ...
              {filteredSuggestions.map((suggestion, index) => (
                ...
                >
                  ...
                  <div className="text-xs text-gray-500 dark:text-gray-400">
                    ...
                    {suggestion.idDocument && `• ${suggestion.idDocument}`}
                  </div>
                </div>
              ))}
            </div>
